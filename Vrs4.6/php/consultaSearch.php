...
<body>
	...
	<?php
		$consulta = mysql_query("SELECT NOM_INST, FECHA, MUNICIPIO, utc, CODIGO, 'Estudiante'
		                       FROM GUIA_EST 
		                      WHERE GUIA_EST.NOM_INST LIKE '%".$q."%'
		                        AND ESTADO = 'S'
		                      UNION
		                     SELECT NOM_INST, FECHA, MUNICIPIO, utc, CODIGO, 'Docente'
		                       FROM GUIA_DOC 
		                      WHERE GUIA_DOC.NOM_INST LIKE '%".$q."%'
		                        AND ESTADO = 'S'
		                   ORDER BY FECHA DESC");
	...
	?>
			<script type="text/javascript">
				$("#tablaMain").load("php/noResultadoSearch.php");
           	</script>
    <?php
		}else{
	?>
			<h2 class="tituloDep">Resultados de la Busqueda</h2>
	...
	<?php	$resultado = mysql_fetch_array($consulta);
			do{ 
	?>
				<div class='resultado'>
